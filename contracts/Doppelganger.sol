...
//
// Based off of: https://github.com/TrueFiEng/Waffle/blob/master/waffle-mock-contract/src/Doppelganger.sol
...
contract Doppelganger {
    ...
    function __doppelganger__queueRevert(
        bytes memory data,
    ...
    function __doppelganger__mockReverts(
        ...
        __doppelganger__queueRevert(data, reason);
    }

    function __doppelganger__queueReturn(
        bytes memory data,
        bytes memory value
    ) public {
        // get the root node of the linked list for this call
    ...
    function __doppelganger__mockReturns(
        ...
        __clearQueue(keccak256(data));
        __doppelganger__queueReturn(data, value);
    }

    function __doppelganger__receiveReverts(string memory reason) public {
        receiveReverts = true;
        ...
        returns (MockCall memory mockCall)
    {
        ...
        if (mockCall.next != "") {
            ...
            if (mockConfig[mockCall.next].next != "") {
                // basically if it's not the last mock call
    ...
    function __internal__mockRevert(string memory reason) private pure {
        revert(reason);
    }
}
